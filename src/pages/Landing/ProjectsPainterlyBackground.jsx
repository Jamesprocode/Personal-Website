...
function buildRings(count) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const isBand = i % 6 === 0;
    const stroke = isBand ? 2.2 : 0.95;
    // Stronger opacity than v1 — the user wants visible, not whisper.
    const fade = 1 - Math.pow(t, 2.4) * 0.55;
    const opacity = (isBand ? 0.55 : 0.32) * fade;
...
function ProjectsPainterlyBackground({ reduceMotion, scrollYProgress }) {
  ...
  const TRACK_R = 1000;
  const trackScale = r([1680 / TRACK_R, 460 / TRACK_R]);
  ...
  const trackRotate = r([-8, 22]);

  // Dust drift
  const dustY = r([40, -90]);
  const dustOpacity = r([0.7, 0.4]);
      ...
      >
        <defs>
          ...
          <radialGradient id="pj-paper" cx={`${(FX / 1440) * 100}%`} cy={`${(FY / 900) * 100}%`} r="110%">
            <stop offset="0%" stopColor="#f5d99a" stopOpacity="0.55" />
            <stop offset="35%" stopColor="#edcb86" stopOpacity="0.28" />
          ...
          <radialGradient id="pj-corner" cx="50%" cy="50%" r="50%">
            ...
            <stop offset="45%" stopColor="#a06f1d" stopOpacity="0.14" />
