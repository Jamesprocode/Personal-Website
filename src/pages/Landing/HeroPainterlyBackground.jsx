...
function HeroPainterlyBackground({ reduceMotion, scrollYProgress, inView = true }) {
  ...
  const pourScale = r([1, 1.18]);
  const pourRotate = r([0, -4]);
  const pourY = r([0, -36]);
  const coreY = r([0, -64]);
  const coreScale = r([1, 1.08]);
  const tendrilsY = r([0, -110]);
  const tendrilsX = r([0, -28]);
  const tendrilsOpacity = r([0.55, 0.18]);
  // Crackle veins INCREASE in opacity with scroll — ink drying, cracks
  // becoming more visible as the brass ages on the page.
  const cracksOpacity = r([0.45, 0.95]);
